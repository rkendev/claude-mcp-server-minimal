...
which does: `copier copy . /tmp/claude-mcp-server-minimal-scratch-$$ --defaults --trust`
→ `cd` into the scratch dir → `uv sync --all-extras` → `make check`.

**Expected output**

```
→ Scratch dir: /tmp/claude-mcp-server-minimal-scratch-NNNNN
→ Template root: /home/you/claude-mcp-server-minimal
...
→ docker compose up -d --wait ollama
 ✔ Network claude-mcp-server-minimal_default  Created
 ✔ Volume  claude-mcp-server-minimal_ollama   Created
 ✔ Container claude-mcp-server-minimal-ollama  Healthy
...
```bash
uv build
unzip -l dist/*.whl | grep claude_mcp_server_minimal/__init__.py
```

**Expected output**

```
Successfully built dist/claude_mcp_server_minimal-0.1.0.tar.gz
Successfully built dist/claude_mcp_server_minimal-0.1.0-py3-none-any.whl
      401  1980-01-01 00:00   claude_mcp_server_minimal/__init__.py
```

**What it proves**

The `[tool.hatch.build.targets.wheel] packages = ["src/claude_mcp_server_minimal"]`
